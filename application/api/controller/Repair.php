...
class Repair extends Api
{
    ...
    public function list2()
    {
        ...
        $item = ItemModel::field('id,category_id,name,price,image_url')->where('status', '1')->order('id', 'asc')->select()->toArray();
        foreach ($item as $k => $v) {
            $item[$k]['image_url'] = $url . $v['image_url'];
        }
        $data = [];
    ...
    public function findincubbycatag()
    {
        $url = $this->request->domain() . '/static/uploads/';
        $category = CategoryModel::field('id,name')->order('sort', 'asc')->select()->toArray();
        $incub = IncubModel::field('id,category_id,type,code,seriesno,image_url')->where('status', '在用')->order('id', 'asc')->select()->toArray();
        foreach ($incub as $k => $v) {
            $incub[$k]['image_url'] = $url . $v['image_url'];
        }
        $data = [];
        foreach ($category as $v) {
            $data[$v['id']] = array_merge($v, ['incub' => []]);
            foreach ($incub as $vv) {
                if ($v['id'] === $vv['category_id']) {
                    $data[$v['id']]['incub'][$vv['id']] = $vv;
                }
            }
        }
        return json([
    ...
    public function order()
    {
        ...
        $order = OrderModel::get($id, 'OrderItem');
        ...
        foreach ($order['order_Item'] as $k => $v) {
            $item = ItemModel::field('name,image_url')->where(['id' => $v['item_id'], 'status' => '1'])->find();
            if (!$item) {
                continue;
            }
            $order['order_item'][$k]['image_url'] = $url . $item['image_url'];
            $order['order_item'][$k]['name'] = $item['name'];
        }
        foreach ($order['order_Incub'] as $j => $d) {
            $incub = IncubModel::field('name,type,status,price')->where(['id' => $d['incub_id']])->find();
            if (!$incub) {
                continue;
            }
            $order['order_Item'][$j]['name'] = $incub['name'];
            $order['order_Item'][$j]['type'] = $incub['type'];
            $order['order_Item'][$j]['status'] = $incub['status'];
            $order['order_Item'][$j]['price'] = $incub['price'];
        }
        return json($order);
    }

    public function orderbyincub()
    {
        if ($this->request->isPost()) {
//            if ($this->request->post('id/d', 0)) {
//                return $this->commentOrder();
//            }
            return $this->createOrderbyincub();
        }
        $id = $this->request->get('id/d', 0);
...
//        }
        ...
        $url = $this->request->domain() . '/static/uploads/';
        foreach ($order['order_Incub'] as $j => $d) {
            $incub = IncubModel::field('code,type,status,seriesno,image_url')->where(['id' => $d['incub_id']])->find();
            if (!$incub) {
                continue;
            }
            $order['order_incub'][$j]['code'] = $incub['code'];
            $order['order_incub'][$j]['type'] = $incub['type'];
            $order['order_incub'][$j]['status'] = $incub['status'];
            $order['order_incub'][$k]['image_url'] = $url . $incub['image_url'];
            $order['order_incub'][$j]['seriesno'] = $incub['seriesno'];
        }
        return json($order);
    }
    ...
    protected function createOrder()
    {
        ...
        $number = 0;
        $order_food = [];
        $food_data = ItemModel::field('id,category_id,name,price,image_url')->where('id', 'in', array_keys($food_ids))->where('status', '1')->select()->toArray();
        ...
        OrderModel::get($order->id)->orderItem()->saveAll($order_food);
    ...
    public function orderlist()
    {
        ...
        foreach ($list as $k => $v) {
            $item_id = OrderItemModel::where('order_id', $v['id'])->limit(1)->value('item_id');
            $list[$k]['first_item_name'] = ItemModel::where('id', $item_id)->value('name');
        }
        return json(['list' => $list, 'last_id' => $last_id]);
    }

    public function orderincublist()
    {
        $order_id = $this->request->get('order_id/d', 0);
            $incubid_list = OrderIncubModel::where('order_id', $order_id)->value('item_id');
            $list = ItemModel::where('id', $incubid_list)->select();

        return json(['incublist' => $list]);
    }
    ...
    protected function createOrderbyincub()
    {
        $order = $this->request->post('order/a', []);
        $comment = $this->request->post('comment/s', '', 'trim');
        $incub_ids = [];
        foreach ($order as $v) {
            $incub_ids[(int)$v['id']] = (int)$v['number'];
        }

        $number = 0;
        $order_incub = [];
        $incub_data = IncubModel::field('id,category_id,type,image_url')->where('id', 'in', array_keys($incub_ids))->where('status', '1')->select()->toArray();
        foreach ($incub_data as $v) {
            $order_incub[$v['id']] = [
                'incub_id' => $v['id'],
                'number' => $incub_ids[$v['id']]
            ];
            $number += $order_incub[$v['id']]['number'];
        }
        $order = [
            'user_id' => $this->user['id'],
            'number' => $number,
        ...
        $order = OrderModel::create($order);
        foreach ($order_incub as $k => $v) {
            $order_incub[$k]['order_id'] = $order->id;
        }
        OrderModel::get($order->id)->orderIncub()->saveAll($order_incub);
        return json(['order_id' => $order->id]);
    }
